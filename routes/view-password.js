...
router.get('/',checkLoginUser ,async function(req, res, next) {
   ...
   await passwordModule.paginate({},options).then(function(result){
      console.log(result);
      res.render('view_all_category', { title: 'Password category',loginUser:loginUser,records:result.docs,current: result.offset,
  ...
  router.get('/:page',checkLoginUser ,function(req, res, next) {
    ...
    .limit(perPage).exec(function(err,data){
      if(err) throw err;
      passwordModule.estimatedDocumentCount({}).exec((err,count)=>{ 
  ...
  router.post('/edit/:id',checkLoginUser, function(req, res, next) {
    ...
    passwordModule.findByIdAndUpdate(id,{password_name:passcat,project_name:project_name,password_detail:pass_details}).exec(function(err){
    if(err) throw err;
      var getPassDetails=passwordModule.findById({_id:id});
    getPassDetails.exec( function(err,data){
  if(err) throw err;
